...
bash /airgeddon/airgeddon.sh
ls

# 2 or 3 of these is for airgeddon, about 3 are for wifite but
# the rest are for stuff that I probably will never remember 
wget -qO - https://raw.githubusercontent.com/sml156/t4uh-setup/master/add-packages-list.txt > /add-packages/add-packages-list.txt 1>/dev/null && echo "OK add-packages Downloaded"  || echo "add-packages Failed"
apt -y install $(cat /add-packages/add-packages-list.txt | xargs)
